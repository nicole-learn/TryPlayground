...
const LOCAL_EVENTS_PULSE_MS = 1200;
const LOCAL_EVENTS_KEEPALIVE_MS = 15000;
...
export async function GET(request: Request) {
  try {
    ...
    if (
      ...
      createStudioRouteError(400, "The local event revision was invalid.");
    }
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        const scheduleNextTick = () => {
          ...
          tickTimeout = setTimeout(runTick, LOCAL_EVENTS_PULSE_MS);
        };
        ...
        const runTick = async () => {
          ...
          try {
            const payload = await getLocalSyncPayload(
              currentRevision,
              providerSettings
            );
    ...
    return toStudioErrorResponse(error, "Could not open local realtime events.", 400);
  }
}
